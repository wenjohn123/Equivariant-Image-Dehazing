import os
import sys
sys.path.append("EI-main/utils")

import torch
from torch.optim import Adam, SGD
from deepinv.models.unet import UNet

from deepinv.models.restormer import Restormer

# from deepinv.models.discriminator import Discriminator
##import models.discriminator.Discriminator
...
from deepinv.utils.logger import get_timestamp, LOG

import models

def check_params_updated(model, name):
    for param_name, param in model.named_parameters():
        if param.grad is not None:
            print(f"参数 {name}.{param_name} 已更新")
        else:
            print(f"参数 {name}.{param_name} **未更新**（可能未参与计算图或被冻结）")
...
class EI(object):
    ...
    def train_ei(self, dataloader, physics, transform, epochs,
        ...
        generator = UNet(in_channels=self.in_channels, out_channels=self.out_channels, residual=residual, scales=5, circular_padding=True, cat=cat).to(self.device)
        if pretrained:
        ...
        optimizer = Adam(generator.parameters()
                         , lr=lr['G'], weight_decay=lr['WD'])

        if report_psnr:
            log = LOG(save_path, filename='training_loss',
                      field_name=['epoch', 'loss_mc', 'loss_ei', 'loss_total', 'NIQE', 'BRISQUE'])
        else:
            log = LOG(save_path, filename='training_loss',
        ...
        for epoch in range(epochs):
            ...
            if report_psnr:
                print('{}\tEpoch[{}/{}]\tmc={:.4e}\tei={:.4e}\tloss={:.4e}\tNIQE={:.4f}\tBRISQUE={:.4e}'.format(get_timestamp(), epoch, epochs, *loss))
            else:
                print(
    ...
    def train_ei_adv(self, dataloader, physics, transform, epochs,
        ...
        discriminator = models.Discriminator((self.in_channels, self.img_width, self.img_height))
